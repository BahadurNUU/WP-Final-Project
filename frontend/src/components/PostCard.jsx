...
export default function PostCard({ post }) {
  ...
  const { userId, username } = useContext(AuthContext);
    ...
    setIsLiked(!isLiked);
    const data = await request(`/api/posts/${post.id}/like`, 'POST')
	};

	const handleBookmark = async () => {
    setIsBookmarked(!isBookmarked);
    
    const data = await request(`/api/bookmarks/${userId}/${post.id}`, 'POST')
	...
	const handleCommentSubmit = async (e) => {
      ...
      const data = await request('/api/comments', 'POST', comment);
      if (data) {
